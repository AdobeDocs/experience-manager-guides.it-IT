...
2. Aggiorna la proprietà `<dox.version>` nel file `/dox/dox.installer/pom.xml` del codice Git dei servizi cloud a 2023.6.297.
3. Apporta le modifiche ed esegui la pipeline dei servizi cloud per l’aggiornamento alla versione di giugno 2023 di AEM Guides as a Cloud Service.
...
In questa sezione è elencata la matrice di compatibilità per le applicazioni software supportate dalla versione di AEM Guides as a Cloud Service di giugno 2023.
...
- Editor web | Il percorso di output e il modello non possono essere selezionati nel predefinito di AEM. (11530)
