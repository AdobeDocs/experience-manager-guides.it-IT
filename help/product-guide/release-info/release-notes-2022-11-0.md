...
1. Aggiorna la proprietà `<dox.version>` nel file `/dox/dox.installer/pom.xml` del codice Git dei servizi cloud a 2022.11.198.
1. Apporta le modifiche ed esegui la pipeline dei servizi cloud per l’aggiornamento alla versione di novembre di AEM Guides as a Cloud Service.
...
Questi predefiniti globali vengono visualizzati nella scheda **Output** di tutte le mappe correlate. Puoi utilizzarli per generare l’output per tutte le mappe correlate. Per generare l&#39;output di PDF, potete selezionare il predefinito come predefinito PDF predefinito di default. Puoi anche **Modificare**, **Rinominare**, **Duplicare** o **Eliminare** un predefinito di output esistente dal menu **Opzioni**.
...
* PDF nativo | `conkeyref` non viene risolto nell&#39;output PDF generato. (10564)
* PDF nativo | Si verificano problemi durante l’accesso ai metadati di una mappa nell’output PDF. (10556)
...
* La pubblicazione in PDF non riesce se creiamo un predefinito tramite la duplicazione di un predefinito esistente. (10584)
...
* Quando si utilizza un&#39;esportazione nativa di PDF, `<indexterm>` nidificati non sono nidificati nell&#39;indice. (10521)
