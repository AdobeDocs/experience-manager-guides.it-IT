...
| **Incorpora file multimediali** | Seleziona questa opzione per incorporare le risorse multimediali supportate, come audio e video, direttamente nel PDF generato. Quando questa opzione è attivata, il contenuto video viene rappresentato in PDF tramite un&#39;immagine poster o una miniatura e può essere riprodotto in visualizzatori PDF compatibili che supportano elementi multimediali incorporati. Per la riproduzione di un video, apri PDF in un’applicazione supportata, ad esempio l’ultima versione di Adobe Acrobat, e, quando richiesto, considera attendibile il documento. La maggior parte dei visualizzatori PDF basati su browser non supporta la riproduzione di contenuti multimediali incorporati. |
...
| **Mostra filigrana** | Selezionare questa opzione per sovrapporre una filigrana nell&#39;output. È possibile immettere una nuova stringa di testo nella casella di testo con il carattere maiuscolo nel modo desiderato. <br><br>Utilizzare variabili di testo statiche o di lingua per pubblicare la versione localizzata della filigrana.  A seconda della lingua scelta, il valore localizzato viene selezionato automaticamente nell’output di PDF. Ad esempio, è possibile stampare Publisher come filigrana in inglese e Auteure in francese.  <br> Formato: `${lng:<variable name>}`. Ad esempio, `$ {lng:publisher-label}` dove `publisher-label` è una variabile di linguaggio. |
