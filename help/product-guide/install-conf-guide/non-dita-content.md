...
- [Documenti di InDesign](#id195AD0B0K5Z)
...
### Convertire i documenti di Word esistenti in documenti di tipo argomento DITA

Nelle schede seguenti vengono fornite istruzioni per convertire i documenti di Word esistenti in documenti di tipo argomento DITA in base alla configurazione di Experience Manager Guides: Cloud Service o On-Premise.
...
1. Dopo aver configurato i parametri richiesti nel file `w2d_io.xml`, accedere ad AEM e aprire l&#39;interfaccia utente di Assets.

1. Passare alla cartella di input \(`wordtodita`\).

1. Caricare i documenti Word di origine in questa cartella. Per informazioni sul caricamento di contenuto in DAM, vedere [Caricare contenuto DITA esistente](upload-dita-content.md#).
...
In generale, il processo di conversione richiede di [preparare i file InDesign per la conversione](aemg-appendix.md#id195DBF0045Z) e [preparare il file di mappatura per la migrazione da InDesign a DITA](aemg-appendix.md#id194AF0003HT), quindi è necessario seguire la procedura specificata di esecuzione del processo di conversione.
...
1. Creare un nodo di sovrapposizione della cartella `config` all&#39;interno del nodo `apps`.
   ...
   - Nell&#39;elemento `mapStyle` specificare la posizione del file di mapping che contiene i mapping per gli stili di documento InDesign agli elementi DITA. La mappatura predefinita viene memorizzata nel file che si trova in:
     ...
     >[!NOTE]
     >
     > Per ulteriori informazioni sulla struttura del file `stmap.adobeidml.xml` e su come personalizzarlo, vedere [Appendice](aemg-appendix.md#id194AF0003HT).

1. Salva il file `idml2dita_io.xml`.

1. Dopo aver configurato i parametri richiesti nel file `idml2dita_io.xml`, accedere ad AEM e aprire l&#39;interfaccia utente di Assets.

1. Passare alla cartella di input \(`indesigntodita`\).

1. Carica i documenti InDesign di origine in questa cartella. Per informazioni sul caricamento di contenuto in DAM, vedere [Caricare contenuto DITA esistente](upload-dita-content.md#).
...
### Converti il documento XHTML esistente in argomento DITA

Le schede seguenti forniscono istruzioni per convertire il documento XHTML esistente in argomento DITA in base alla configurazione di Experience Manager Guides: Cloud Service o On-Premise.
...
1. Carica i documenti XHTML di origine in questa cartella. Per informazioni sul caricamento di contenuto in DAM, vedere [Caricare contenuto DITA esistente](upload-dita-content.md#).
...
1. Dopo aver configurato i parametri richiesti nel file `h2d_io.xml`, accedere ad AEM e aprire l&#39;interfaccia utente di Assets.
...
Dopo la conversione, è possibile migrare il contenuto strutturato in AEM Guides. Per ulteriori dettagli, visualizzare [Caricare contenuto DITA esistente utilizzando lo strumento WebDAV e FrameMaker](./upload-dita-content.md).
...
Le schede seguenti forniscono istruzioni per eseguire la migrazione di qualsiasi altro documento strutturato in base alla configurazione di Experience Manager Guides: Cloud Service o On-Premise.
...
1. Dopo aver configurato i parametri richiesti nel file `XSLConfig.xml`, accedere ad AEM e aprire l&#39;interfaccia utente di Assets.

1. Passare alla cartella di input \(`xsltodita`\).

1. Carica i documenti strutturati di origine in questa cartella. Per informazioni sul caricamento di contenuto in DAM, vedere [Caricare contenuto DITA esistente](upload-dita-content.md#).
