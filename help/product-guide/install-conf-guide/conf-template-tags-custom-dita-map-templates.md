...
Le schede seguenti forniscono istruzioni per configurare un modello di mappa DITA personalizzato in base alla configurazione di Experience Manager Guides: Cloud Service o On-Premise.
...
## Personalizzare il numero di riferimenti in una mappa DITA (solo per Cloud Service)
...
Utilizza le istruzioni fornite in [Sostituzioni configurazione](download-install-config-override.md#) per creare il file di configurazione. Nel file di configurazione, specificare i dettagli (proprietà) riportati di seguito per specificare il numero di riferimenti nel modello di mappa DITA per mantenere il processo sincrono.
