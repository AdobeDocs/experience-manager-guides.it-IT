...
- **Configurazioni editor XML**: utilizzare questa scheda per personalizzare l&#39;aspetto e le varie caratteristiche dell&#39;editor Web. Per l&#39;editor Web sono disponibili le seguenti impostazioni configurabili:
   ...
   - Configurazione dell&#39;editor XML solo per Cloud Service
   - Layout modello CSS
   - Snippet editor XML
...
**Usa attributo personalizzato**

Se si utilizza un attributo personalizzato, deve essere un attributo DITA valido supportato dalla DTD. Nelle schede seguenti vengono fornite istruzioni per l&#39;utilizzo di qualsiasi attributo, che non è un attributo DITA standard in base alla configurazione di Experience Manager Guides: Cloud Service o On-Premise.
...
1. Utilizzare Gestione pacchetti per scaricare il file `/libs/fmdita/config/condAttrList.csv`. A seconda della versione in uso, anche il file `libs/fmdita/config/condAttrList.xml` potrebbe essere disponibile per il download.

1. Creare una copia del file `condAttrList.csv` nel percorso seguente nell&#39;archivio Git di Cloud Manager:
...
1. Passare al file `condAttrList.csv` disponibile nel percorso seguente:

   `/libs/fmdita/config/condAttrList.csv`

   A seconda della versione in uso, il file `libs/fmdita/config/condAttrList.xml` potrebbe essere disponibile nella stessa posizione.

1. Creare un nodo di sovrapposizione della cartella `config` all&#39;interno del nodo `apps`.

1. Passare agli attributi personalizzati del file condAttrList.xml nel nodo `apps` e aggiungerli:
...
Le schede seguenti forniscono istruzioni per configurare i modelli di authoring in base alla configurazione di Experience Manager Guides: Cloud Service o On-Premise.
...
AEM Guides viene fornito con 7 modelli di authoring predefiniti e 2 modelli di mappe DITA. Puoi scegliere di avere solo pochi modelli disponibili per gli autori. Se utilizzi un modello personalizzato, lo stesso può essere configurato e reso disponibile per l’authoring. La scheda Modello di creazione della configurazione Profili cartella consente di aggiungere o rimuovere modelli di argomento o di mappa da profili globali o a livello di cartella.

Anche prima di configurare i modelli di argomento o mappa a livello globale o di cartella, puoi definire una posizione in cui memorizzare i modelli di authoring personalizzati. Per configurare un percorso personalizzato per l&#39;archiviazione dei modelli di creazione, vedere [Configurare il percorso della cartella dei modelli DITA personalizzati](conf-template-tags-custom-dita-topic-template.md#id191LCF0095Z).
...
1. Nella pagina del profilo, fare clic sulla scheda **Modello di creazione**.
1. Fai clic su **Modifica**.
...
Oltre ai predefiniti configurati a livello globale o aziendale, un editore disporrebbe comunque dei diritti per creare più predefiniti di output. Tuttavia, tali predefiniti sono associati alla mappa DITA per la quale vengono creati. Per ulteriori dettagli sulla creazione di predefiniti di output regolari per una mappa DITA a seconda della configurazione in uso, vedere *Creare, modificare, duplicare o rimuovere un predefinito di output* in Utilizzo di Adobe Experience Manager Guides.
...
1. Per creare o modificare un predefinito di output, effettuate una delle seguenti operazioni:
   ...
   - Fai clic su **Modifica** per aprire la configurazione del predefinito selezionato per la modifica.
     ...
     Per informazioni sulle impostazioni dei predefiniti di output in base alla configurazione in uso, vedere *Informazioni sui predefiniti di output* in Utilizzo di Adobe Experience Manager Guides.
...
Le seguenti schede forniscono informazioni in base alla configurazione di Experience Manager Guides: Cloud Service o On-Premise.
...
Questa impostazione controlla la barra degli strumenti e gli altri elementi dell’interfaccia utente dell’editor. Seleziona l&#39;icona **Scarica** per scaricare il file `ui\_config.json` nel sistema locale. Puoi quindi apportare modifiche al file e al caricamento allo stesso modo. A seconda della posizione in cui stai caricando il file, a livello globale o a livello di cartella, le modifiche vengono applicate di conseguenza. Per ulteriori dettagli su come personalizzare l&#39;editor XML utilizzando `ui\_config.json file`, visualizzare [Personalizza barra degli strumenti](customize-toolbar.md#).
