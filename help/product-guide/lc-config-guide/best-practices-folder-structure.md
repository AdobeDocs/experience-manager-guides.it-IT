...
La cartella `fm-ditaoutputs` funge da percorso di archiviazione predefinito per gli output generati dai contenuti di apprendimento e formazione. Questi output in genere includono pacchetti SCORM (file ZIP) nella cartella **alm** e PDF nella cartella **pdf**.Se necessario, puoi modificare questo percorso di output predefinito a livello di predefinito dalla **console Mappa**.
