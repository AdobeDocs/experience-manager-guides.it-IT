...
| **Mostra filigrana** | Selezionare questa opzione per sovrapporre una filigrana nell&#39;output. È possibile immettere una nuova stringa di testo nella casella di testo con il carattere maiuscolo nel modo desiderato. <br><br>Utilizzare variabili di testo statiche o di lingua per pubblicare la versione localizzata della filigrana.  A seconda della lingua scelta, il valore localizzato viene selezionato automaticamente nell’output di PDF. Ad esempio, è possibile stampare Publisher come filigrana in inglese e Auteure in francese.  <br> Formato: `${lng:<variable name>}`. Ad esempio, `$ {lng:publisher-label}` dove `publisher-label` è una variabile di linguaggio. <br> Passa il cursore sopra <img src="./assets/info-details.svg" alt= "icona info" width="25"> vicino all&#39;opzione per visualizzare ulteriori dettagli su di essa. |
