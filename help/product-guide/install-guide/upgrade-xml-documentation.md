...
Puoi aggiornare la versione corrente di Experience Manager Guides alla versione 5.0.0:


- Se utilizzi le versioni 4.6.x, 4.6 o 4.4, puoi eseguire direttamente l’aggiornamento alla versione 5.0.0.
...
1. Disinstallare eventuali hotfix di Experience Manager Guides installati sulla versione precedente \(major o patch release\) di Experience Manager Guides.

**Prima di installare la versione 4.0**

Prima di installare la versione 4.0, attenersi alla seguente procedura:

1. Verifica a questo punto che Experience Manager Guide sia sulla versione 3.8.5.
1. Scarica il pacchetto script di aggiornamento. A tale scopo, ricerca &quot;XML Documentazione solution 4.0 Upgrade Package&quot; sul [portale](https://experience.adobe.com/#/downloads/content/software-distribution/it/aem.html) Adobe Systems di distribuzione software che scaricare un file zip.
1. Carica questo pacchetto su AEM via Gestione pacchetti e installa questo pacchetto.
1. Una volta installato il pacchetto di aggiornamento, eseguire gli script indicati di seguito nello stesso ordine e seguire le istruzioni fornite:

**Verifica la compatibilità dell&#39;aggiornamento API**
...
| Risposta attesa | - Una volta completata la migrazione, è possibile installare XML Documentazione soluzione versione 4.0.<br>- In caso di errori, ripristinare l&#39;ultimo checkpoint e condividere i log degli errori insieme all&#39;output API con il team di successo del cliente. |
...
Prima di avviare il processo di aggiornamento di Experience Manager Guides 4.2, assicurarsi di:

1. Aggiornamento alle guide Experience Manager versione 4.0, 4.1 o 4.1.x.
1. Ha chiuso tutte le attività di traduzione.
...
1. Dopo aver completato l&#39;installazione del pacchetto, attendi il seguente messaggio \(s\) nei registri:

   `Completed the post deployment setup script`

   Il messaggio sopra indica che tutti i passaggi dell&#39;installazione sono stati completati.

   Nel caso in cui si verifichi uno dei seguenti prefissi ERROR, segnalali al team di successo del cliente:

   - Errore nello script di installazione dell post installazione
   - Eccezione durante il trasferimento della MAP di traduzione
   - Impossibile porta mappa di traslazione da v1 a v2 per la proprietà
...
1. Configura il **passaggio del processo** con i dettagli seguenti:
   ...
   **Descrizione**: DXML post processo iniziatore passaggio che attiverà un lavoro di imbracatura per l&#39;elaborazione post DXML del risorsa modificato/creato
...
- L&#39;API restituirà un jobId. Per controllare lo stato del processo, puoi inviare un richiesta GET con ID processo allo stesso endpoint -
...
1. Nell&#39;indice oak damAssetLucene, aggiungi la proprietà `indexNodeName` booleana come `true` nel nodo.
   `/oak:index/damAssetLucene/indexRules/dam:Asset`
1. Aggiungere un nuovo nodo con il nome estratto sotto il nodo.

   `/oak:index/damAssetLucene/indexRules/dam:Asset/properties`e impostare le seguenti proprietà nel nodo:
   ...
   La struttura di dovrebbe apparire qualcosa di `damAssetLucene` like:
...
1. Reindicizzare l&#39;indice `damAssetLucene` (impostando la reindicizzazione contrassegno come `true` in
e attendi che lo sia `false` di nuovo (questo indica che la reindicizzazione è completa). Si noti che potrebbero essere necessarie alcune ore a seconda delle dimensioni dell&#39;indice.
...
1. Fai clic su **Sincronizza** in alto a destra dopo aver completato le modifiche. Riceverai una notifica di successo.

   >[!NOTE]
   >
   > Aggiorna e verifica che le modifiche personalizzate e la fase di elaborazione di post Guide Experience Manager siano presenti nel modello di workflow finale.

1. Una volta **convalidato DAM workflow** Aggiorna risorsa, verificate le configurazioni di modulo di avvio corrispondenti. A tale scopo, vai all AEM interfaccia del flusso di lavoro e apri i moduli di avvio.

   ```http
   http://localhost:4502/libs/cq/workflow/content/console.html
   ```

   Trova e apporta modifiche \(se necessario\) ai seguenti due moduli di avvio \(che dovrebbero essere attivi\) corrispondenti a **DAM workflow** Aggiorna risorsa:
...
1. Modulo di avvio per &quot;*Nodo creato*&quot; per **Flusso di lavoro Aggiorna risorsa DAM**- per la condizione `"jcr:content/jcr:mimeType!=video"`, il valore &#39;Globbing&#39; deve essere:
   ...
   - Launcher per &quot;*Nodo modificato*&quot; per **DAM Aggiorna risorsa workflow -** per la condizione &quot;`jcr:content/jcr:mimeType!=video`&quot;, il valore &#39;Globbing&#39; dovrebbe essere:
...
1. Esegui le seguenti API per eseguire l&#39;elaborazione post su tutti i file:

   | Punto finale | /bin/guides/reports/upgrade |
   |---|---|
   | Tipo di richiesta | **POST** Questo script è una richiesta POST, quindi deve essere eseguito tramite agenti come Postman. |
   | Risposta prevista | L’API restituirà un jobId. Per controllare lo stato del processo, puoi inviare un richiesta GET con ID processo allo stesso endpoint.<br> URL di esempio: `http://<server:port>/bin/guides/reports/upgrade` |

   | Punto finale | /bin/guides/reports/upgrade |
   |---|---|
   | Tipo di richiesta | **GET** |
   | Parametro | jobId: passa il jobId ricevuto dal richiesta post precedente. |
...
1. Se il **componente DXML Post Process Initiator** è presente, assicurati che le personalizzazioni siano sincronizzate.
1. Se il **componente DXML Post Process Initiator** è assente, esegui i seguenti passaggi per inserirlo:

1. Fare clic su **Inserisci componente** \(Responsabile dell Experience Manager elaborazione delle guide post come fase finale del processo\).
1. Configura la fase **del** processo con i dettagli seguenti:
   ...
   **Processo scheda**
   ...
   - Seleziona **DXML Post Process Initiator** dal **menu a discesa Processo**
...
1. Selezionate **DAM Aggiorna risorsa workflow**.
1. Fai clic su **Modifica**.
1. Se il componente **DXML Post Process Initiator** è presente, verificare che le personalizzazioni siano sincronizzate.
1. Se il componente **DXML Post Process Initiator** è assente, eseguire la procedura seguente per inserirlo:
...
2. Impostare la proprietà reindex su `true` (`false` per impostazione predefinita) e fare clic su **Salva Tutto**.
...
- Eseguire un richiesta POST al server \(con autenticazione corretta\) - `http://<server:port\>/bin/guides/map-find/indexing`. (Facoltativo: è possibile passare percorsi specifici delle mappe per indicizzarle, per impostazione predefinita tutte le mappe saranno indicizzate || Esempio : `https://<Server:port\>/bin/guides/map-find/indexing?paths=<map\_path\_in\_repository\>`)

- L’API restituirà un jobId. Per verificare lo stato del processo, potete inviare un richiesta GET con ID processo allo stesso endpoint - `http://<server:port\>/bin/guides/map-find/indexing?jobId=\{jobId\}`\(ad esempio: ` http://localhost:8080/bin/guides/map-find/indexing?jobId=2022/9/15/7/27/7dfa1271-981e-4617-b5a4-c18379f11c42_678`)
...
>[!TIP]
>
> L’aggiornamento alla versione 5.0.0 dipende dalla versione corrente di Experience Manager Guides. Se utilizzi le versioni 4.6.3, 4.6.1, 4.6.0 o 4.4, puoi eseguire direttamente l’aggiornamento alla versione 5.0.0.
...
1. Scaricare il pacchetto versione 5.0.0 dal [Adobe Systems Software Distribution Portal](https://experience.adobe.com/#/downloads/content/software-distribution/it/aem.html).
...
1. Launcher per &quot;*Nodo creato*&quot; per **DAM Aggiorna risorsa workflow**- per la condizione `"jcr:content/jcr:mimeType!=video"`, il valore &#39;Globbing&#39; dovrebbe essere:
...
1. Al termine dell’aggiornamento, accertati che le personalizzazioni/sovrapposizioni siano convalidate e aggiornate in modo che corrispondano al nuovo codice dell’applicazione. Di seguito sono riportati alcuni esempi:
   - Tutti i componenti sovrapposti da / libs / fmditaor / libs dovrebbero essere confrontati con il nuovo codice prodotto e gli aggiornamenti dovrebbero essere fatti in file sovrapposti sotto / apps.
   - Tutte le categorie clientlib utilizzate dal prodotto, devono essere riviste per eventuali modifiche. Eventuali configurazioni sovrascritte \(esempi sotto\) devono essere confrontate con quelle più recenti in modo da ottenere le funzionalità più recenti:
   - elementmapping.xml
   - ui\_config.json\(potrebbe essere stato impostato in profili cartella\)
   - ha modificato `com.adobe.fmdita.config.ConfigManager`
...
- Una volta completato il lavoro, il richiesta GET di cui sopra risponderà con successo e menzionerà se alcune mappe non sono riuscite. Il successo delle mappe indicizzate può essere confermato dai log del server.
