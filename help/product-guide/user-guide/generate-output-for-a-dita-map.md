...
1. Nell’interfaccia utente Assets, accedi al file di mappa DITA che desideri pubblicare e fai clic su di esso.
...
1. Selezionate uno o più predefiniti di output da utilizzare per generare l&#39;output.
   ...
   >[!NOTE]
   >
   > Se stai generando l’output del sito AEM, il processo di pubblicazione utilizza la struttura definita nella sezione `.ditamap` per creare la struttura del sito AEM.

1. Fai clic sull’icona Genera per avviare il processo di generazione dell’output.


Puoi visualizzare lo stato corrente della richiesta di generazione dell’output facendo clic su Output. Per ulteriori informazioni, consulta [Visualizza lo stato dell&#39;attività di generazione output](#viewing_output_history)
...
È possibile generare l&#39;output PDF per uno o più predefiniti di output creati per una mappa DITA dall&#39;Editor Web. Per ulteriori dettagli, consulta [Utilizza il pannello Generazione rapida per generare e visualizzare l’output dei predefiniti](web-editor-quick-generate-panel.md#).

È inoltre possibile generare l&#39;output del sito AEM per uno o più argomenti o per l&#39;intera mappa DITA dall&#39;editor Web. Per ulteriori dettagli, consulta [Pubblicazione basata su articolo dall’editor web](web-editor-article-publishing.md#id218CK0U019I).
...
1. Clic **Rigenera** per avviare il processo di generazione dell&#39;output.
...
Dopo aver avviato l&#39;operazione di generazione output per una mappa o rigenerato gli argomenti selezionati, Guide AEM invia l&#39;operazione alla coda di generazione output. Questa coda viene aggiornata in tempo reale, mostrando lo stato di ogni operazione di generazione output nella coda.

Per visualizzare la coda di generazione dell’output, effettua le seguenti operazioni:

1. Nell’interfaccia utente Assets, passa a e fai clic sul file di mappa di cui desideri controllare lo stato di generazione dell’output.
...
Le guide AEM offrono agli editori un modo semplice per annullare qualsiasi attività di pubblicazione in corso. In qualità di editore, puoi annullare un’attività di pubblicazione in corso dalla console delle mappe DITA o dal [Pubblica dashboard](generate-output-publish-dashboard.md#).

Per annullare un&#39;attività di generazione output dalla console delle mappe DITA, effettuare le seguenti operazioni:

1. Nell’interfaccia utente Assets, passa a e fai clic sul file di mappa di cui vuoi annullare un’attività di generazione output in corso.

1. Clic **Uscite**.

1. Nell&#39;elenco Output in coda, posizionare il puntatore del mouse su un&#39;attività che si desidera annullare.

1. Fai clic su *Annulla questo processo* icona.
...
1. Clic **Sì** al prompt dei messaggi Conferma annullamento.
   ...
   Una volta annullata, l&#39;attività viene spostata nel **Uscite generate** elenco con **Annullato** stato. Quando passi il cursore del mouse sull’attività annullata, viene visualizzato il nome dell’utente che ha annullato l’attività. Nella schermata seguente, *HTML5* attività annullata.
...
Quando si generano più output per una mappa DITA, in un periodo di tempo l&#39;elenco Output generati per tale mappa diventa molto lungo. In qualità di editore, è possibile cancellare la cronologia di output di qualsiasi file di mappa rimuovendo le attività obsolete dal *Uscite generate* elenco. L’output non viene rimosso dal sistema, ma solo la voce dell’output generato viene rimossa dal *Uscite generate* elenco.

Per rimuovere un task di output dall&#39;elenco Output generato, effettuare le operazioni riportate di seguito.

1. Nell’interfaccia utente Assets, accedi e fai clic sul file di mappa da cui desideri eliminare le attività.
...
1. Clic **Sì** al messaggio Conferma eliminazione.
